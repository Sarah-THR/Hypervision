<template>
  <v-card class="mx-auto mb-5 pt-5" elevation="3" min-height="800px" height="100%">
    ...
    <div v-if="sector && sector.users">
      ...
      <ul>
        <li v-for="user in sector.users" :key="user.email" class="ms-6 ma-2">{{ user.name }}</li>
      </ul>
      <h4 class="ms-6 ma-2">Accès</h4>
      <ul>
        <li v-for="access in sector.access" :key="access.name" class="ms-6 ma-2">
